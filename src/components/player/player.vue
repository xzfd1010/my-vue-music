  ...
  export default {
    ...
    methods: {
      ...
      onProgressBarChange(percent) {
        ...
        this.$refs.audio.currentTime = this.currentSong.duration * percent // 通过改变currentTime触发变化
        // 拖拽后继续播放
        if (!this.playing) {
          this.togglePlaying()
        }
        if (this.currentLyric) {
          this.currentLyric.seek(currentTime * 1000) // currentTime以s计算，*1000为ms
        }
      },
      end() {
        // 应该根据播放模式切换到下一首
      ...
      middleTouchMove(e) {
        ...
        const left = this.currentShow === 'cd' ? 0 : -window.innerWidth // 歌词部分距离屏幕右侧的宽度，起始位置
        const offsetWidth = Math.min(0, Math.max(-window.innerWidth, left + deltaX)) // (-innerWidth,0)之前的值
        this.touch.percent = Math.abs(offsetWidth / window.innerWidth) // 获取滑动的百分比=滑动距离/屏幕宽度
      ...
      _getPosAndScale() {
        // 计算距离的方法
        const targetWidth = 40 // 底部图标的宽度
        const paddingLeft = 40 // 底部图标的padding-left
        const paddingBottom = 30 // 底部图标的中心点距离底部的高度
        const paddingTop = 80 // cd距离顶部的高度
        const width = window.innerWidth * 0.8 // cd的宽度
        const scale = targetWidth / width // 初始的缩放比例
        const x = -(window.innerWidth / 2 - paddingLeft) // 横向移动的距离
